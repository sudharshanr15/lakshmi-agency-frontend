...
export function Desktopnav() {
  ...
  useEffect(() => {
    const closeMenuOnClickOutside = (event) => {
      if (isDrawerOpen && !event.target.closest("#drawer-navigation")) {
        setDrawerOpen(false);
          ...
          {isDrawerOpen && (
            <ul className="py-5">
              {categList.map((item) => (
                <li
                  key={item.id}
                  className="px-3 space-x-5 flex py-3 text-gray-700 justify-between  hover:bg-gray-200 cursor-pointer"
                  type="button"
                  onMouseEnter={() => {
      ...
      {selectedCategory && (
        ...
        >
          <button
            ...
            aria-controls="drawer-navigation"
            className="text-gray-400 bg-transparent hover:bg-gray-200  rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center  dark:hover:text-white"
          >
            <svg
              aria-hidden="true"
              className="w-5 h-5"
          ...
          <ul className="py-4">
            {selectedCategory === "Pipes" &&
              pipes.map((item) => (
                <li
                  key={item.id}
                  className="px-3 space-x-5 flex py-5 text-gray-700 justify-between  hover:bg-gray-200"
                >
                  <div className="flex">
