...
function DetailsTab({ setDetailsTab, detailsTab }) {
  return (
    <div className="absolute top-0 left-0 h-full w-full z-[500]">
      <div className="w-full h-full flex">
        ...
        <div className="bg-white h-full w-[90%] lg:w-[50%] z-[110] relative p-4">
          ...
          {/* contents */}
          <Content detailsTab={detailsTab} setDetailsTab={setDetailsTab} />
        </div>
      </div>
    </div>
  );
...
function Content({ detailsTab, setDetailsTab }) {
  if (detailsTab == "change-address") {
    return <SelectAddress setDetailsTab={setDetailsTab} />;
  } else {
    return <AddAddress setDetailsTab={setDetailsTab} />;
  }
}
